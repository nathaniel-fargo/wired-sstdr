...
%   'positive_only' - Return only 0 to +t instead of -t to +t (default: false)
...
addParameter(p, 'positive_only', false, @islogical);  % NEW: Return only 0 to +t
...
if strcmp(cfg.method, 'time') || strcmp(cfg.method, 'both')
    ...
    % Apply positive_only filter if requested
    if cfg.positive_only
        positive_indices = time_lags_time >= 0;
        correlation_time = correlation_time(positive_indices);
        time_lags_time = time_lags_time(positive_indices);
        lags_time = lags_time(positive_indices);
        fprintf('  Applied positive_only filter: %d samples (0 to +t)\n', length(correlation_time));
    end
...
if strcmp(cfg.method, 'freq') || strcmp(cfg.method, 'both')
    ...
    % Apply positive_only filter if requested
    if cfg.positive_only
        positive_indices = time_lags_freq >= 0;
        correlation_freq = correlation_freq(positive_indices);
        time_lags_freq = time_lags_freq(positive_indices);
        lags_freq = lags_freq(positive_indices);
        fprintf('  Applied positive_only filter: %d samples (0 to +t)\n', length(correlation_freq));
    end
